...
tryCatch({
  ...
  if (nrow(scrape_oneway$data) > 0) {
    ...
    cat("  - Average price: $", round(mean(scrape_oneway$data$price, na.rm = TRUE), 2), "\n", sep = "")
    cat("  - Min price: $", min(scrape_oneway$data$price, na.rm = TRUE), "\n", sep = "")
    cat("  - Max price: $", max(scrape_oneway$data$price, na.rm = TRUE), "\n", sep = "")
    cat("  - Direct flights:", sum(scrape_oneway$data$num_stops == 0, na.rm = TRUE), "\n")
    cat("  - Flights with stops:", sum(scrape_oneway$data$num_stops > 0, na.rm = TRUE), "\n")
...
tryCatch({
  ...
  if (nrow(scrape_roundtrip$data) > 0) {
    cat("\n✓ Successfully scraped", nrow(scrape_roundtrip$data), "total flights\n")
    ...
    tryCatch({
      ...
      # List cached files
      cached_files <- list.files(cache_dir, pattern = "\\.csv$", full.names = TRUE)
...
cat("  • Use realistic future dates (Google Flights typically shows ~6-12 months ahead)\n")
