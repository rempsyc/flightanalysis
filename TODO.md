...
Make all data processing functions only work with `flight_results`
objects (no backward compatibility):
...
- Replace `sample_flights` usage with `sample_flight_results` where
  applicable
...
- Clarify that `sample_flights` is kept for demonstration but main
  functions require `flight_results`
