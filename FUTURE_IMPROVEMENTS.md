...
Implement sample datasets following R package conventions to enable
offline testing and better documentation examples.
...
**6. Update documentation:** - Add note in README.md about sample
datasets under “Usage” section - Update examples in man pages to
optionally use sample data - Consider adding to vignettes if they exist
...
`data/` directory contains 3 `.rda` files: `sample_query.rda`,
`sample_flights.rda`, `sample_multi_origin.rda`
...
Man pages (`?sample_query`, `?sample_flights`, `?sample_multi_origin`)
display correctly
...
Consider adding `.Rbuildignore` entry for `^data-raw$` if not already
present
