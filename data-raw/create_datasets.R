...
for (origin in origins) {
  ...
  for (date in as.character(dates)) {
    ...
    travel_time_str <- sprintf(
      "%d hr %d min",
      floor(travel_time_hrs),
      travel_time_min
    )
    ...
    price <- round(
      base_price * spike_factor * weekend_factor + rnorm(1, 0, noise_sd)
    )
    ...
    flights_data <- rbind(
      ...
      data.frame(
        ...
        airlines = sample(
          c("Air India", "United", "Lufthansa", "Emirates", "Turkish Airlines"),
          1
        ),
