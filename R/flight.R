...
#' @examples
#' \dontrun{
#' flight <- Flight("2023-07-20", "JFKIST", "9:00AM", "5:00PM+1", 
...
classify_arg <- function(flight, arg) {
  ...
  if ((grepl("AM", arg) || grepl("PM", arg)) && length(flight$times) < 2 && grepl(":", arg)) {
    delta_days <- 0
  ...
  else if ((grepl("hr", arg) || grepl("min", arg)) && is.null(flight$flight_time)) {
    flight$flight_time <- arg
  }
  # Check for number of stops
  else if (grepl("stop", arg) && is.null(flight$num_stops)) {
    flight$num_stops <- if (arg == "Nonstop") 0 else as.integer(strsplit(arg, " ")[[1]][1])
  }
  # Check for CO2
  ...
  # Check for emissions
    ...
    flight$emissions <- if (emission_val == "Avg") 0 else as.integer(gsub("%", "", emission_val))
  }
  # Check for price
  ...
  else if (nchar(arg) == 6 && arg == toupper(arg) && is.null(flight$origin) && is.null(flight$dest)) {
    flight$origin <- substr(arg, 1, 3)
    flight$dest <- substr(arg, 4, 6)
  }
  # Check for stops with time
  else if ((grepl("hr", arg) && grepl("[A-Z]{3}$", arg)) || 
...
flights_to_dataframe <- function(flights) {
  ...
  for (flight in flights) {
    row <- data.frame(
      departure_datetime = if (!is.null(flight$time_leave)) format(flight$time_leave, "%Y-%m-%d %H:%M:%S") else NA,
      arrival_datetime = if (!is.null(flight$time_arrive)) format(flight$time_arrive, "%Y-%m-%d %H:%M:%S") else NA,
      ...
      emission_diff_pct = ifelse(is.null(flight$emissions), NA, flight$emissions),
      stringsAsFactors = FALSE
    )
    data <- rbind(data, row)
  }
