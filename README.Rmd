...
The package now includes full web scraping functionality using RSelenium! First, install the required packages:
...
# Scrape the data (this will open a Chrome browser)
...
# For headless scraping (no browser window)
ScrapeObjects(scrape, headless = TRUE)
```

The `ScrapeObjects()` function will:
1. **Run pre-flight checks** - Verify Chrome installation and internet connectivity
2. **Automatically set up Chrome driver** - Multiple fallback methods for reliability
...
If you encounter errors, the function will automatically:
- Detect if Chrome/Chromium is installed
- Check internet connectivity
- Try multiple driver initialization methods (rsDriver and wdman)
- Verify driver connections before scraping
